...
export const InputLanguageInlineGrid = ({ name, id, text, endpoint }) => {
  ...
  const uploadItem = async (id, language, value) => {
    ...
    var result = await callAuthApi(endpoint, formValuesBody, "PUT");
...
export const AntDateTimeRangePicker = ({ onChange, startOnToDate, dateFormat, fromDate, toDate, diffDate = 7, emptyAtBegin = false, allowClear = true }) => {
  ...
  const initValue = fromDate && toDate ? [moment(fromDate), moment(toDate)] : startOnToDate ? startOnToDateValue : NotStartOnToDateValue;
  ...
  return (
    <div className="div-ant-range-picker">
      <RangePicker defaultValue={!emptyAtBegin ? cVal : []} format={dateFormat || constants.DatePickerFormat} onChange={onChangeVal} allowClear={allowClear} />
    </div>
  );
};
...
export const InputNum = forwardRef((props, ref) => {
  ...
  const handleChange = val => {
    if (max > 0 && val && val.floatValue > max) val = { ...val, floatValue: max };
...
export const TableSearch = ({ onSearch, helpText, placeholder }) => {
  ...
  const handleChange = evt => {
    const value = evt.target.value;
    if (value === "") searchOnClear("");
    else searchOnClear(evt.target.value);
  };
