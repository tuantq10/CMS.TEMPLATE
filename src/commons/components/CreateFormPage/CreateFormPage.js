...
import { getIdFromPath, validPermission } from "~/commons/utils/function";
import useReactRouter from "use-react-router";
import "./CreateFormPage.less";

const CreateFormPageX = ({
  currentRoutePath,
  UpsertPopup,
  langPrefix,
  upsertExtraParams,
  ...
  onDelete,
  forwardCallback,
  callbackIsReadOnlyForm,
  className,
  forwardRef
}) => {
  const { t } = useTranslation();
  const { history } = useReactRouter();
  const initialUpsertPopupState = {
    id: "",
    submit: false,
    clear: false,
    delete: false,
  ...
  const [upsertPopupState, setUpsertPopupState] = useState(
    initialUpsertPopupState
  );
            ...
            {(!upsertPopupState.hideClearBtn ||
              ...
              <Fragment>
                ...
                <div className="div_nk_form_button_area">
                  {!!onDelete && (
                    <Button onClick={handleConfirmDelete} skin="destructive">
                      {t("general.btnDelete")}
                    </Button>
                  )}
                  {!upsertPopupState.hideClearBtn && (
                    <Button onClick={onUpsertPopupClear}>
                      {t(
                        `general.${
                          isDetailPage ? "btnClearAndCreateNew" : "btnClear"
                        }`
                      )}
                    </Button>
                  )}
                  {!hideSubmitBtn && (
                    <Button onClick={onUpsertPopupSubmit} type="primary">
                      {t("general.btnSave")}
                    </Button>
                  )}
                </div>
