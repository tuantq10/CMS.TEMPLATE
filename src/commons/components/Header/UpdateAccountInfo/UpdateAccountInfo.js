...
import { LoadingPanel } from "../../CustomComponents/CustomComponents";
...
import { alertMessage, isValidComplexity, validateFormInput, buildReturnUrl, getErrorMessage } from "../../../../commons/utils/function";
...
export const UpdateAccountInfo = ({isSubmitButtonClick, callbackSubmitted}) => {
    ...
    const validateRule = {
        requireds: ['firstName', 'lastName', 'phoneNumber'],
        validate: [
            {
                field: 'phoneNumber',
                func: (val, vals) => {
            ...
            },
            {
                ...
                func: (val, vals) => {
                    const isInputCurrentPassword = vals && vals['oldPassword'] && vals['oldPassword'].length > 0;
                    const newPassword = vals && vals['password'];
                    if ((!val || val.length <= 0) && isInputCurrentPassword) {
                        return t('general.isRequired');
        ...
        <Fragment>
            ...
            <Form onSubmit={handleSubmitFrm} layout="horizontal">
                <Row gutter={48}>
                    ...
                    <Col span={17}>
                        <InputText focus={true} value={formValues.firstName} onChange={handleChangeVal} error={formErrors.firstName} name="firstName" onSubmit={handleSubmit}/>
                    </Col>
                </Row>
                <Row gutter={48}>
                    <Col span={7} className="form-title">
                    ...
                    <Col span={17}>
                        <InputText value={formValues.lastName} onChange={handleChangeVal} error={formErrors.lastName} name="lastName" onSubmit={handleSubmit}/>
                    </Col>
                </Row>
                <Row gutter={48}>
                    <Col span={7} className="form-title">
                    ...
                    <Col span={17}>
                        <InputText focus={true} value={formValues.username} disabled={true}/>
                    </Col>
                </Row>
                <Row gutter={48}>
                    <Col span={7} className="form-title">
                ...
                <Collapse bordered={false}>
                    <Panel header="Change Password" className="change-password" key="1" extra={<Icon type="key"/>}>
                        ...
                        <Row gutter={48}>
                            ...
                            <Col span={17}>
                                <InputPassword value={formValues.password} onChange={handleChangeVal} error={formErrors.password} name="password" onSubmit={handleSubmit}/>
                            </Col>
                        </Row>
                        <Row gutter={48}>
                            <Col span={7} className="form-title">
