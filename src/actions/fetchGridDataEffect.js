...
import { callAuthApi, authApi } from '../utils/apiCaller';
import { alertMessage, detectReturnMessage, buildQueryStringFetchData } from '../utils/function';
...
export const fetchGridDataEffect = (fetchUrl, removeUrl, sortColumnMapping, callbackRemoved, initialQueryParams, initialData, onDeleted) => {
    ...
    const fetchData = async (abortController) => {
        ...
        try {
            if (fetchUrl) {
                if (!queryParams.sortBy && sortColumnMapping && Object.keys(sortColumnMapping).length > 0)
                    queryParams.sortBy = `${sortColumnMapping[0]} ASC`;
    ...
    const changeSortBy = (sortCol, isDesc) => {
        ...
        const sortBy = sortField && sortField !== '' ? sortField + (isDesc ? ' DESC' : ' ASC') : '';

        queryParams.sortBy !== sortBy && setQueryParams({...queryParams, sortBy: sortBy, pageIndex: 1});
    };

    const changeKeySearch = (keySearch, evt) => {
        queryParams.keySearch !== keySearch && setQueryParams({...queryParams, keySearch: keySearch, pageIndex: 1});
