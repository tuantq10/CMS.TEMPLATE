...
import {
  ...
  buildMenuFromSimpleToComplex,
  getErrorMessage
} from "../../commons/utils/function";
      ...
      callApi(endpoint.auth, null, data, "POST").then(res => {
        ...
        if (login.isSuccess) {
          setLoginLocalStorage({
            ...
            [`${constants.TokenExpiration}`]: moment
              .utc(res.data.expires)
              .local()
              .format(constants.DateTimeFormatFromDB)
          });
          callAuthApi(endpoint.profile)
            .then(res => {
              if (res.status === 200) {
                ...
                const data = buildMenuFromSimpleToComplex(
                  routes,
                  userPermissions
                );
                if (data && data[0]) {
                  ...
                  if (firstRoute) {
                    const route =
                      firstRoute.permissions &&
                      firstRoute.permissions.length > 0
                        ? firstRoute
                        : firstRoute.routes[0];
                    if (route) {
                      redirectRoute = constants.Slash + route.route;
                      localStorage.setItem(
                        constants.DefaultPathName,
                        redirectRoute
                      );
                    }
                  }
